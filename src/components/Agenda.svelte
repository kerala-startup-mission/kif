<script>
  ...
  <div class="agenda__tabs space-x-4 text-center text-kif-blue mb-5 ">
    {#each Object.entries(agenda_list) as [date, agendas]}
      <button on:click={()=> selectTab(date)} class="px-5 py-3 spl_cursor mb-3 sm:text-base text-xs rounded-md shadow  border {date == active_tab ? 'bg-kif-red text-white border-red-600' : 'border-kif-blue'} " >
  ...
  <div class="agenda__tabs space-x-4 text-center text-kif-blue mb-5 ">
    {#each Object.entries(venues) as [index,venue]}
      <button on:click={()=> selectVenue(venue)} class="px-5 py-3 mb-3  spl_cursor sm:text-base text-xs rounded-md shadow  border {venue == active_venue ? 'bg-kif-red text-white border-red-600' : 'border-kif-blue'} " >
        <span>{venue}
      </button>
    {/each}
  </div>
